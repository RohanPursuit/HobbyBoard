...
const getAllProjectConnections = async ({ project_id }) => {
  try {
    const allProCons = await db.any(
      "SELECT connections.username,permissions,connections.project_id,users.profile_image FROM connections JOIN users ON connections.username=users.username WHERE connections.project_id=$1",
      project_id
    );
    return allProCons;
  } catch (err) {
